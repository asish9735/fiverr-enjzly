...
class Proposals extends MX_Controller {
	...
	public function edit_proposal($proposal_id='',$token='')
	{
		...
		if($verify_token==$token){
			$data['load_js']=load_js(array('croppie.js','summernote.js','tagsinput.js','upload-drag-file.js'));
			$data['load_css']=load_css(array('tagsinput.css','croppie.css','summernote-bs4.css'));
	...
	public function makefeature($id){
		...
		if($check_proposal){
			...
			elseif($method=='bank'){
				...
				if($seller_details && $seller_details['member']->bank_transfer_allowed==1){
					$wallet_transaction_type_id=get_option_value('FEATURED_PAYMENT_BANK');
					$current_datetime=date('Y-m-d H:i:s');
					$wallet_transaction_id=insertTable('wallet_transaction',array('wallet_transaction_type_id'=>$wallet_transaction_type_id,'status'=>1,'created_date'=>$current_datetime,'transaction_date'=>$current_datetime),TRUE);
					if($wallet_transaction_id){
						...
						$insert_wallet_transaction_row=array('wallet_transaction_id'=>$wallet_transaction_id,'wallet_id'=>$seller_wallet_id,'credit'=>$total,'description_tkey'=>'Online_payment_from','relational_data'=>'Bank');
						$insert_wallet_transaction_row['ref_data_cell']=json_encode(array(
							'FW'=>$bank_details->title,
							'TW'=>$seller_details['member']->member_name.' wallet',	
							'TP'=>'Wallet_Topup',
	...
	public function offercheckout($id){
		...
		if($check_proposal){
			...
			elseif($method=='ngenius'){
				$feeCalculation=generateProcessingFee('ngenius',$offer_fee);
				$processing_fee=$feeCalculation['processing_fee'];
				$total=$offer_fee+$processing_fee;
				$order_time = date("M d, Y H:i:s", strtotime(" + ".$check_proposal->delivery_time." days"));
				$OrderData=array(
	...
	public function postproposalCheckAjax(){
		...
		$i=0;
		$msg=array();
		$minimum_service_value=get_option_value('minimum_service_value');
		if($this->input->post()){
		fromVRules('proposal_title', 'title', 'required|trim|xss_clean|min_length[15]|max_length[80]');
		fromVRules('proposal_description', 'description', 'required|trim|xss_clean');
		...
		fromVRules('is_fixed', 'price type', 'required|trim|xss_clean|numeric');
		$is_fixed=0;
		if($this->input->post('is_fixed') && $this->input->post('is_fixed')=='1'){
			fromVRules('proposal_price', 'price', 'required|trim|xss_clean|numeric|greater_than_equal_to['.$minimum_service_value.']');
			$is_fixed=1;
		}else{
			...
			fromVRules('package_price_1', 'price', 'required|trim|xss_clean|numeric|greater_than_equal_to['.$minimum_service_value.']');
			fromVRules('package_price_2', 'price', 'required|trim|xss_clean|numeric|greater_than_equal_to['.$minimum_service_value.']');
			fromVRules('package_price_3', 'price', 'required|trim|xss_clean|numeric|greater_than_equal_to['.$minimum_service_value.']');
		}
		...
		if($this->input->post('attribute_count')){
			...
			$allattr=array_unique($attribute_count);
				...
				foreach($allattr as $k=>$value){
					...
					for($p=1;$p<=3;$p++){
					fromVRules('attribute_value_'.$p.'_'.$value, 'attribute value', 'required|trim|xss_clean');
		...
		if (isVRulePassed() == FALSE){
			...
			$proposal_url=generateProjectSlug(trim(strip_tags(post('proposal_title'))));
			$arr=array(
					'select'=>'p.proposal_id',
					'table'=>'proposals p',
			...
			if($i==0){
				...
				if($proposal_id){
					...
					updateTable('proposals',array('proposal_price'=>post('proposal_price'),'display_price'=>post('proposal_price')),array('proposal_id'=>$proposal_id));
						...
						'proposal_id'=>$proposal_id,
						'package_name'=>'Basic',
						'price'=>post('package_price_1'),
						'delivery_time'=>post('package_time_1'),
						'description'=>post('package_desc_1'),
						);
						$packageIDONE=insertTable('proposal_packages',$proposal_packages,TRUE);
						$proposal_packages=array(
						'proposal_id'=>$proposal_id,
						'package_name'=>'Standard',
						'price'=>post('package_price_2'),
						'delivery_time'=>post('package_time_2'),
						'description'=>post('package_desc_2'),
						);
						$packageIDTWO=insertTable('proposal_packages',$proposal_packages,TRUE);
						$proposal_packages=array(
						'proposal_id'=>$proposal_id,
						'package_name'=>'Advance',
						'price'=>post('package_price_3'),
						'delivery_time'=>post('package_time_3'),
						'description'=>post('package_desc_3'),
						);
						$packageIDTHREE=insertTable('proposal_packages',$proposal_packages,TRUE);
						...
						updateTable('proposals',array('display_price'=>$display_price,'proposal_price'=>0),array('proposal_id'=>$proposal_id));	
					}
					if($attribute_name){
					...
					if($attribute_name){
						foreach($allattr as $k=>$value){
							...
							for($p=1;$p<=3;$p++){
								...
								create_update('proposal_package_attributes',$insdata,array('attribute_name'=>$attribute_name,'package_id'=>$package_id,'proposal_id'=>$proposal_id));
							}
						}
					}
					$check=$this->db->select('category_module')->from('category')->where('category_id',post('category_id'))->get()->row();
					if($check){
						if($check->category_module==1){
							$section_data=array(
								'logo_transparency','vector_file','printable_file','3d_mockup','source_file','social_media_kit','no_of_concept','revisions'
							);
							if($is_fixed!=1){
								$this->db->where('proposal_id',$proposal_id)->delete('proposal_package_module_attributes');
								if($section_data){
									foreach($section_data as $sectioninput){
										for($p=1;$p<=3;$p++){
											if($p==1){
												$package_id=$packageIDONE;
											}elseif($p==2){
											...
											$attribute_value=($this->input->post($sectioninput.'_'.$p) ? $this->input->post($sectioninput.'_'.$p):'');
											$proposal_package_module_attributes=array('package_id'=>$package_id,'proposal_id'=>$proposal_id,'attribute_key'=>$sectioninput,'attribute_value'=>$attribute_value);
											insertTable('proposal_package_module_attributes',$proposal_package_module_attributes);
										}
									}
								}
							}
					...
					$SELLER_NAME=getUserName($this->member_id);
...
/*					$proposal_stat=array(
	...
	public function view($username,$project_url){
		...
		$data['other_proposal']=getData(array('select'=>'p.proposal_id,p.proposal_seller_id,p.proposal_title,p.proposal_price,p.display_price,p.proposal_url,p.proposal_image,p_set.proposal_featured,p_set.featured_end_date,m.member_name,m.seller_level,p_set.proposal_enable_referrals',
				'table'=>'proposals p',
				'join'=>array(
	...
	public function favorites(){
		...
		$data['all_favorites']=getData(array('select'=>'p.proposal_id,p.proposal_seller_id,p.proposal_title,p.proposal_price,p.display_price,p.proposal_url,p.proposal_image,p_set.proposal_featured,p_set.featured_end_date,m.member_name,m.seller_level,p_set.proposal_enable_referrals',
				'table'=>'favorites f',
				'join'=>array(
	...
	public function getextra(){
		...
		$dataid=get('Okey');
		$pid=get('pid');
		$check=$this->db->select('category_module')->from('category')->where('category_id',$dataid)->get()->row();
		if($check){
			$all_attr_values=array();
			if($pid){
				$all_attr_valuesData=$this->db->select('attribute_value,attribute_key')->from('proposal_package_module_attributes')->where('proposal_id',$pid)->order_by('package_id','asc')->get()->result();
				if($all_attr_valuesData){
					foreach($all_attr_valuesData as $k=>$attr){
						$all_attr_values[$attr->attribute_key][]=$attr->attribute_value;
					}
				}
			}
			if($check->category_module==1){
				$section_data=array(
					'section'=>1,
					'all_attr_values'=>$all_attr_values,
				);
				$templateLayout=array('view'=>'extra-option','type'=>'ajax','buffer'=>true,'theme'=>'');
				$response['section_1']=load_template($templateLayout,$section_data);
				$section_data=array(
					'section'=>2,
					'all_attr_values'=>$all_attr_values,
				);
				$response['section_2']=load_template($templateLayout,$section_data);
				$section_data=array(
					'section'=>3,
					'all_attr_values'=>$all_attr_values,
				);
				$response['section_3']=load_template($templateLayout,$section_data);
