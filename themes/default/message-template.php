	...
	<li v-for="chat_user in chatUserList" :class="{'active-message': active_chat && (chat_user.conversations_id==active_chat.conversations_id)}">
		<a href="#" @click.prevent="setActiveChat(chat_user)">
			...
			<div class="message-by">
				...
				<p v-if="chat_user.unread_msg_count > 0"><b><span v-html="chat_user.message"></span></b> <span class="badge badge-site">{{chat_user.unread_msg_count}}</span></p>
...
Vue.component('chat-list', {
  ...
  methods: {
	loadChat: function(){
		var _self = this;
		$.getJSON('<?php echo base_url('message_new/load_chat')?>', function(res){
			if(res.status == 1){
...
<div class="messages-headline">
	<a :href="active_chat.profile_url" target="_blank">
		<div class="user-avatar" :class="{'status-online': active_chat.online_status, 'status-offline': !active_chat.online_status}">
			<img :src="active_chat.avatar" alt="" />
		</div>
		<h4 class="ml-2">{{active_chat.name}}</h4>
	</a>
			...
			<div class="message-bubble" :class="{'me': message.sender_id == login_user.member_id}">
				<div class="message-bubble-inner">
					...
					<div class="message-text" :class="{'message-deleted': message.is_deleted}">
						...
						<div v-if="message.offer_id"><!--- message-offer Starts --->
							
						<div class="message-offer card mb-1">
								<div class="card-header">
									<h6 class="mt-md-0 mt-2 mb-0">
									{{message.offer_data.proposal_title}}
									<span class="price float-right d-sm-block d-none"> <?php D(CURRENCY); ?>{{message.offer_data.amount}}</span>
									</h6>
								</div>
								<div class="card-body"><!--- card-body Starts --->
									<p>{{message.offer_data.description}}</p>
									<p class="d-block d-sm-none"> <b> <?php D(__('message_board_page_offer_template_price','Price / Amount :'));?> </b> {{message.offer_data.amount}} </p>
									<p><b><i class="icon-feather-calendar"></i> <?php D(__('message_board_page_offer_template_Delivery_time','Delivery Time :'));?> </b> {{message.offer_data.delivery_time}} days</p>
									<div v-if="message.offer_data.status==0">
										<button v-if="message.offer_data.offer_sender!=login_user.member_id" @click="acceptOffer(message)" :id="'accept-offer-'+message.offer_id" class="btn btn-success float-right"><?php D(__('message_board_page_Accept_Offer','Accept Offer'));?></button>
									</div>
									<div v-if="message.offer_data.status==1">
										<button class="btn btn-sm btn-success mr-2" disabled><?php D(__('message_board_page_Offer_Accepted','Offer Accepted'));?></button> <a :href="'<?php D(get_link('OrderDetailsURL'))?>'+message.offer_data.order_id" target="_blank" class="btn btn-sm btn-outline-site"><?php D(__('message_board_page_View_Order','View Order'));?></a>
									</div>
									
								</div>
							</div>

						</div><!--- message-offer End --->
						...
						<div class="fixed-action-btn" v-if="message.is_deleted == null && message.offer_id==null">
...
Vue.component('active-chat-body', {
  ...
  methods: {
        ...
        bootbox.confirm({
			...
			callback: function (result) {
				if(result==true){
					...
					$.post('<?php echo base_url('message_new/edit_ajax')?>', {message: $message.val().trim(), ID: msg.message_id}, function(res){
						if(res.status == 1){
    ...
    deleteMsg: function(msg){
        bootbox.confirm({
			...
			callback: function (result) {
				if(result==true){
					$.post('<?php echo base_url('message_new/delete_msg')?>/'+msg.message_id, function (res) {
						if(res.status == 1){
    ...
    starMessage: function(e, msg){
       ...
       $.post('<?php echo base_url('message_new/star_toggle'); ?>', {ID: msg.message_id, type: 'message'}, function(res){
            if(res.status > 0){
                if(res.action == 'added'){
                    msg.starred = 1;
                }else{
	...
	loadMessage: function(){
		...
		_self.message_list = [];
		$.getJSON('<?php echo base_url('message_new/load_chat_message')?>/'+conversations_id, function(res){
			if(res.status == 1){
	...
	infiniteHandler: function($state){
		var _self = this,
			conversations_id = _self.active_chat.conversations_id;
		$.getJSON('<?php echo base_url('message_new/load_chat_message')?>/'+conversations_id, {limit: _self.next_limit}, function(res){
			if(res.status == 1){
				if(res.chat_message.length > 0){
					res.chat_message.forEach(function(item){
						if(item.attachment != null){
	...
	loadNewMessage: function(){
		var _self = this,
			conversations_id = _self.active_chat.conversations_id;
		$.getJSON('<?php echo base_url('message_new/load_new_message')?>/'+conversations_id, function(res){
			if(res.status == 1){
	...
	acceptOffer: function(msg){
        console.log(msg);
		$.ajax({
			method: "POST",
			url: "<?php D(get_link('MessageOfferAccept'));?>",
			data: {single_message_id: msg.conversations_id, offer_id: msg.offer_id}
		})
		.done(function(data){
  ...
  mounted: function(){
	seller_id=this.active_chat.member_id;
	message_group_id=this.active_chat.conversations_id;
	//this.loadMessage();  
  },
  updated: function(){
...
Vue.component('reply-chat', {
  ...
  methods: {
	...
	sendAttachment(){
		...
		var sendAttachentURL = '<?php echo base_url('message_new/send_attachment'); ?>';
	...
	sendMsg: function(){
		...
		_self.sent_status = 'sending';
		$.ajax({
			url: '<?php echo base_url('message_new/send_msg')?>',
			data: msg_data,
			type: 'POST',
			dataType: 'JSON',
			success: function(res){
...
Vue.component('conversation-attachment', {
  ...
  methods: {
	...
	loadAttachment: function($state){
		var _self = this,
			conversations_id = _self.active_chat.conversations_id;
		$.getJSON('<?php echo base_url('message_new/load_attachments')?>/'+conversations_id, {limit: _self.next_limit}, function(res){
			if(res.status == 1){
