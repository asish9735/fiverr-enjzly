...
<div class="messages-container margin-top-0">
    ...
    <div class="messages-container-inner">
        ...
        <div class="messages-inbox" <?php if(!$selected_room){echo 'style="display:block"';}?>>

            <div class="messages-headline">

            	

                <div class="input-with-icon search-bar">

                        <input type="text" class="form-control" id="autocomplete-input" placeholder="Search">

                    <i class="icon-material-outline-search"></i>

                </div>

                <div class="dropdown mt-2"><!--- dropdown float-left mt-1 Starts --->

					<a class="dropdown-toggle btn btn-outline-secondary btn-block" href="#" data-toggle="dropdown"><?php D(__('message_board_page_All_Conversations',"All Conversations"));?></a>
        ...
        if($conversation_group){
            ...
            foreach($conversation_group as $c=>$group){
				...
				$is_online=is_online($group->chatwith);
                ?>

                

                <li class="<?php D($group->conversations_id);?> <?php D($selected); ?> <?php D($starred); ?> <?php D($archived); ?>" data-username="<?php D($group->chatmember->member_name);?>">

                

                    <a href="javascript:void(0)" data-id="<?php D($group->conversations_id);?>" class="message-recipients">

                        <div class="message-avatar"><i class="status-icon status-<?php echo ($is_online==1 ?'online':'offline');?>"></i><img src="<?php D($group->chatmember->logo);?>" alt="" /></div>
                        ...
                        <div class="message-by">
                            ...
                            <div class="message-by-headline">
                                ...
                                <h5><?php D($group->chatmember->member_name);?> </h5>

                                <span><?php D(get_time_ago($group->sending_date)); ?></span>

                            </div>

                            <p><?php D(substr($message,0,40)); ?></p>
...
<?php /*?>
  ...
  <div class="row mr-0 ml-0 mt-sm-0 mt-md-4 mb-md-4 box-inbox">
		...
		<div class="card border-0 rounded-0 m-0">
	  		...
	  		<div class="card-header bg-transparent inboxHeader">
				...
				<div class="search-bar d-none"><!--- search-bar Starts --->
					...
					<div class="input-group"><!--- input-group Starts --->
	      				...
	      				<div class="input-group-append"><span class="input-group-text"> <a href="javscript:void(0)" class="text-danger"><i class="icon-feather-x"></i><?php // D(__('global_Close',"Close"));?></a> </span></div>
				...
				<div class="float-right mb-1"><!--- float-right mb-1 Starts --->
					...
					<a href="javscript:void(0)" class="text-muted search-icon"> <i class="fa fa-lg fa-search"></i> </a>
			...
			<div class="card-body p-0">
				...
				?>
				    ...
				    <img src="<?php D($group->chatmember->logo);?>" class="rounded-circle" width="50">
	...
	<div class="specfic_ col-md-9 p-0">
		...
		<div id="msgHeader" class="test card-header bg-transparent inboxHeader2 d-none">
				...
				<form id="proposal-details-form"><!--- proposal-details-form Starts --->
					...
					<div class="selected-proposal p-3"><!--- selected-proposal p-3 Starts --->
						...
						<div class="form-group clearfix "><!--- form-group Starts --->
							...
							<label class="font-weight-bold float-left"> <?php D(__('modal_submit_proposal_details_Delivery_Time',"Delivery Time :"));?>  </label>
						...
						<div class="form-group clearfix"><!--- form-group Starts --->
							...
							<label class="font-weight-bold float-left"> <?php D(__('modal_submit_proposal_details_Total_Offer_Amount',"Total Offer Amount :"));?>  </label>
...
<div id="message_template" style="display: none">
	...
	<div class="message-bubble {SENT_BY} message_{MESSAGE_ID}">

    <div class="message-bubble-inner">

        <div class="message-avatar">{SENDER_LOGO}</div>

        <div class="message-text">

        <p>{SENDER_NAME} <small>{SENT_DATE}</small> {REPORT}</p>
...
<div id="offer_template" style="display: none">
	...
	<div class="message-offer card mb-1"><!--- message-offer Starts --->

		<div class="card-header">

			<h6 class="mt-md-0 mt-2 mb-0">
		...
		<div class="card-body"><!--- card-body Starts --->
			...
			<p><small> <b> <i class="icon-feather-calendar"></i> <?php D(__('message_board_page_offer_template_Delivery_time','Delivery Time :'));?> </b> {DELIVERY_TIME}</small> </p>
				...
				if (res['conversation'].length>0) {
					...
					for (x in res['conversation']) {
						...
						if(res['conversation'][x] ['sender_id']=='<?php D($log_member_id)?>'){
							...
							template=template.replace(/{REPORT}/g,' <a href="#" data-toggle="modal" data-target="#report-modal" class="text-muted"><i class="icon-feather-flag"></i> <?php // D(__('message_board_page_Report','Report'));?></a>');
						...
						template=template.replace(/{SENDER_LOGO}/g, '<img src="'+serder_logo+'" alt="" class="rounded-circle" width="40"/>');
						...
						if(res['conversation'][x] ['attachment']){
							...
							if(inarray > -1){
								...
								var messagecontent='<a download class="d-block" target="_blank" href="<?php D(URL_USERUPLOAD.'message-files/')?>'+res['conversation'][x] ['attachment']+'"><i class="icon-feather-download"></i> <img src="<?php D(URL_USERUPLOAD.'message-files/')?>'+res['conversation'][x] ['attachment']+'" alt="" Class="img-thumbnail" width=150/></a>'

							}else{

								var messagecontent='<a download class="d-block" target="_blank" href="<?php D(URL_USERUPLOAD.'message-files/')?>'+res['conversation'][x] ['attachment']+'"><i class="icon-feather-download"></i> '+'<img src="<?php echo theme_url().IMAGE;?>file-type/'+ext+'.svg" alt="" width=48 />'+'</a>';
						...
						if(res['conversation'][x] ['offer_id']){
							...
							if(res['conversation'][x] ['status']==0){
								...
								var offeraction='<button class="btn btn-sm btn-success mr-2" disabled><?php D(__('message_board_page_Offer_Accepted','Offer Accepted'));?></button><a href="<?php D(get_link('OrderDetailsURL'))?>'+res['conversation'][x] ['order_id']+'" class="btn btn-sm btn-outline-site"><?php D(__('message_board_page_View_Order','View Order'));?></a>';
