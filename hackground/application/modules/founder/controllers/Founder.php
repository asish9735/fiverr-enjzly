...
class Founder extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('founder_model', 'founder');
		$this->data['table'] = 'founder';
		$this->data['lang_table'] = 'founder_names';
		$this->data['primary_key'] = 'founder_id';
		...
		$this->founder->configure($model_configuration);
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Founder Management';
		$this->data['second_title'] = 'All Founder List';
		$this->data['title'] = 'Founder';
		$breadcrumb = array(
			array(
				'name' => 'Founder',
				'path' => '',
		...
		$this->data['list'] = $this->founder->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->founder->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->founder->getDetail($id);
			$this->data['title'] = 'Edit Founder';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('founder_name', 'name', 'required');
			$this->form_validation->set_rules('status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->founder->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->founder->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function upload_file(){
		if($_FILES && $this->input->is_ajax_request()){
			$upload_dir = LC_PATH.'userupload/founder/';
			...
			if(! $this->upload->do_upload('file')){
				...
				$this->api->data('file_url', USER_UPLOAD.'founder/'.$this->upload->data('file_name'));
			}
