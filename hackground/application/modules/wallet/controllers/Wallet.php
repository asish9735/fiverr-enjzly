				...
				foreach($list as $k=>$v){	
					...
					if(is_object($relational_data)){
						if($v['description_tkey']=='Bank_Transfer'){
							$details.=' Bank Name : '.$relational_data->bname."\t\n";
							$details.=' Swift Code : '.$relational_data->bcode."\t\n";
							$details.=' IBAN : '.$relational_data->to."\t\n";
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				...
				if($sts==2){
					...
					$wallet_id=get_setting('PROCESSING_FEE_WALLET');
					$total_debit = $this->wallet->wallet_debit_balance($wallet_id);
					$total_credit = $this->wallet->wallet_credit_balance($wallet_id);
					$org_balance = $total_credit + $total_debit;
					update_wallet_balance($wallet_id, $org_balance);
					...
					'content'=>json_encode(array('TID'=>$ID)),
					);
