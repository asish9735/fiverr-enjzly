   ...
   private $data;
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('category_key', 'category key', 'required|regex_match[/^[a-z\-A-Z-0-9_]+$/]|is_unique[category.category_key]');
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('status', 'status', '');
			$this->form_validation->set_rules('category_key', 'category key', 'required|regex_match[/^[a-z\-A-Z-0-9_]+$/]');
	...
	public function upload_file(){
		if($_FILES && $this->input->is_ajax_request()){
			$upload_dir = LC_PATH.'userupload/category/';
			...
			if(! $this->upload->do_upload('file')){
				...
				$this->api->data('file_url', USER_UPLOAD.'category/'.$this->upload->data('file_name'));
			}
