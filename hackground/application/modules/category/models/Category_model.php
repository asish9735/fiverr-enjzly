...
class Category_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if($for_list){
			$result = $this->db->limit($offset, $limit)->order_by('a.display_order', 'ASC')->order_by('a.'.$this->primary_key, 'DESC')->get()->result_array();
		}else{
	...
	public function addRecord($data=array()){
		$structure = array(
			'category_image' => !empty($data['category_image']) ? $data['category_image'] : '',
			...
			'category_module' => !empty($data['category_module']) ? $data['category_module'] : '0',
		);
		$ins['data'] = $structure;
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			...
			$lang_info[$v['lang']] = $v['info'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['name'] = $lang_name;
			$result['lang']['info'] = $lang_info;
		}
		return $result;
	}
