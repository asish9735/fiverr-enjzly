...
class Sub_category extends MX_Controller {
   ...
   private $data;
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Sub Category Management';
		$this->data['second_title'] = 'Sub Category List';
		...
		$this->data['add_btn'] = 'Add Sub Category';
	...
	public function load_ajax_page(){
		...
		$category_config = array(
			'table' => 'category',
			'lang_table' => 'category_names',
			'primary_key' => 'category_id',
		);
		$this->category_model->configure($category_config);
		
		if($page == 'add'){
			$this->data['category_id'] = get('category_id');
			$this->data['category'] = $this->category_model->getList(null, 0, 1000);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('category_subchild_key', 'category key', 'required|regex_match[/^[a-z\-A-Z-0-9_]+$/]|is_unique[category_subchild.category_subchild_key]');
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('status', 'status', '');
			$this->form_validation->set_rules('category_subchild_key', 'category key', 'required|regex_match[/^[a-z\-A-Z-0-9_]+$/]');
