...
class Nationality extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'nationality';
		$this->data['lang_table'] = 'nationality_names';
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[nationality_name]['.$v.']', "name $v", 'required|trim|max_length[100]');
			}
			
			$this->form_validation->set_rules('nationality_code', 'nationality code', 'required|alpha|is_unique[nationality.nationality_code]');
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('nationality_status' => $sts));
			}else{
				$upd['data'] = array('nationality_status' => $sts);
