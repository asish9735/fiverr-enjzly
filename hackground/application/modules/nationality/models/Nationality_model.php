...
class Nationality_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.nationality_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.nationality_status <>', DELETE_STATUS);	
		}
		...
		if(!empty($srch['term'])){
			$this->db->group_start();
			$this->db->like('b.nationality_name', $srch['term']);
			$this->db->group_end();
		}
		
		$this->db->where('b.nationality_lang', $admin_default_lang);	
		if($for_list){
			$result = $this->db->limit($offset, $limit)->order_by('b.nationality_name', 'ASC')->get()->result_array();
		}else{
	...
	public function addRecord($data=array()){
		$structure = array(
			'nationality_code' => !empty($data['nationality_code']) ? $data['nationality_code'] : '',
			'nationality_status' => !empty($data['status']) ? $data['status'] : '0',
		);
		$ins['data'] = $structure;
		...
		$this->insert_lang_data($lang_fields, $data['nationality_code']);
	...
	public function deleteRecord($id=''){
		if($id && is_array($id)){
			return $this->db->where_in($this->primary_key, $id)->update($this->table, array('nationality_status' => DELETE_STATUS));
		}else{
			$ins['data'] = array('nationality_status' => DELETE_STATUS);
			$ins['table'] = $this->table;
			$ins['where'] = array($this->primary_key => $id);
			return  update($ins);
		}
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['nationality_lang']] = $v['nationality_name'];
		}
		$result['lang'] = array();
